...
namespace zerofix
{
    ...
    public class TcpSession : IDisposable
    {
        ...
        protected volatile int _starting;
        ...
        private void TrySend()
        {
            if (_sending)
            {
                Interlocked.Decrement(ref _starting);
                return;
            }
            ...
            while (process)
            {
                ...
                lock (_sendLock)
                {
                    ...
                    if (_sendBufferFlush.IsEmpty)
                    {
                        ...
                        var tmp = new byte[_sendBufferFlush.Size];
                        Array.Copy(_sendBufferFlush.Data, tmp, _sendBufferFlush.Size);
                        _list.Add(tmp);
                    ...
                    {
                        //Console.WriteLine("//{2} - {0},{1}", _sendBufferFlushOffset, _sendBufferFlush.Size, _iteration);
                        Interlocked.Decrement(ref _starting);
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    {
                        process = ProcessSend(_sendEventArg, true);
                    }
                    //else
                    //{
                    //    //Console.WriteLine("////{2} - {0},{1} - {3}", _sendBufferFlushOffset, _sendBufferFlush.Size, _iteration, _starting);
                    //}
                }
                catch (ObjectDisposedException) {}
            }
        ...
        /// </summary>
        private bool ProcessSend(SocketAsyncEventArgs e, bool sync)
        {
            if (!IsConnected)
                return false;
            ...
            //lock (_sendLock)
            {
                if (size > 0)
                {
                    ...
                    //if (size > _sendBufferFlush.Size)
                    //{
                    //    Console.WriteLine("---{2} - {0},{1} [{3}, {4}, {5}]", _sendBufferFlushOffset, _sendBufferFlush.Size, _iteration, size, sync, _starting);
                    //    Console.WriteLine("---{1}. - [{0}]", _list[_iteration-1].Length, _iteration);
                    //    Console.WriteLine("---{1}.. - [{0}]", _list[_iteration-2].Length, _iteration);
                    //    Console.WriteLine("---{1}... - [{0}]", _list[_iteration-3].Length, _iteration);
                    //}
